...
//  MeetUrFriends
//
//  Created by Mohit Kumar Singh on 01/06/21.
//
...
enum VideoPlayerViewNotification: String {
    case showLoading
    case hideLoading
    ...
    var notificationName: Notification.Name {
        switch self {
        case .showLoading:
            return Notification.Name.init("showloading")
        case .hideLoading:
            return Notification.Name.init("hideLoading")
        }
    }
}

    // other way of writing enum
//enum VideoPlayerViewNotification: NSNotification.Name {
//    case showLoading = Notification.Name.init("showloading")
//    case hideLoading = Notification.Name.init("hideLoading")
//}
...
class VideoPlayerView: UIView {
  ...
  func setupView(){
    ...
    avPlayerLayer.videoGravity = .resizeAspect//.resizeAspectFill
    self.layer.addSublayer(self.avPlayerLayer)
  }
  
  func configure(url: URL?, fileExtension: String?, size: (Int, Int)) {
    self.cancelAllLoadingRequest()
    NotificationCenter.default.post(name: VideoPlayerViewNotification.showLoading.notificationName, object: nil)
    ...
    VideoCacheManager.shared.queryURLFromCache(key: url.absoluteString, fileExtension: fileExtension, completion: {[weak self] (data) in
      DispatchQueue.main.async { [weak self] in
        ...
        if !(self.queuePlayer?.items().contains(self.playerItem!) ?? false) {
          if let queuePlayer = self.queuePlayer {
            guard queuePlayer.canInsert(self.playerItem!, after: queuePlayer.items().last) else {
              return
            }
  ...
  func cancelAllLoadingRequest(){
   // NotificationCenter.default.post(name: VideoPlayerViewNotification.hideLoading.notificationName, object: nil)
    removeObserver()
...
extension VideoPlayerView {
  func isPlaying() -> Bool {
    if (self.queuePlayer?.rate ?? 0) != 0 && self.queuePlayer?.error == nil {
      return true
    }
    return false
  }
